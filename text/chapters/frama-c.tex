...
je následně přeložen na konjunkci $\frac{n(n-2)}{2}$ predikátů typu \texttt{separated(a, b)}~\cite{Correnson2018Separated}.
...
Tabulka~\ref{tab:prepend-smt-success} indikuje zvyšující se složitost důkazu korektnosti funkce \texttt{prepend}
korelující s velikostí seznamu.
Příčinou tohoto problému je,
že kombinace predikátu \texttt{separated} s univerzálním kvantifikátorem
je vysoce neefektivní a náročná jak na výpočetní čas, tak na paměť.

Následující výpis~\ref{list:linked-list-separated-generated-proof}
