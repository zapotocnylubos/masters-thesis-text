...
TODO: pridat info, ze Why3 nepodporuje nejnovejsi verze SMT resicu
%Prover Alt-Ergo version 2.6.1 is not recognized.
%Known versions for this prover: ^2\.5\.[2-9]$.
%Prover CVC4 OK 1.8 (archived)
%Prover CVC5 version 1.2.1 is not recognized.
%Known versions for this prover: ^1\.0\.[0-9]+$.
%Prover Z3 version 4.14.1 is not recognized.
%Known versions for this prover: ^4\.8\.[7-9]$, ^4\.8\.1[0-9]+$,

%Found prover Alt-Ergo version 2.5.4, OK.
%Found prover Alt-Ergo version 2.5.4 (alternative: BV)
%Found prover Alt-Ergo version 2.5.4 (alternative: counterexamples)
...
%Found prover Z3 version 4.8.17 (alternative: counterexamples)
%Found prover Z3 version 4.8.17, OK.
%Found prover Z3 version 4.8.17 (alternative: noBV)
%14 prover(s) added
...
s nově alokovaným ukazatele pomocí přidaného axiomu \texttt{admit} na predikát \texttt{\textbackslash valid(pi)}
a výsledek analyzy je označen jako platný (valid).
Výsledný důkaz zvládl dokonce prokázat interní Qed zjednodušovač.

Více axiomatický přístup k důkazům popisuje kapitola~\ref{subsec:axiomaticky-pristup}.
...
ACSL predikát \texttt{\textbackslash separated} má variabinlní počet parametrů,
...
Tato metoda správně popisuje vstupní podmínky a umožňuje důkaz korektnosti funkce \texttt{prepend},
...
% https://www.frama-c.com/fc-plugins/counter-examples.html