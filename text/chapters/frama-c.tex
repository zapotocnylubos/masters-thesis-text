...
Tento model by měl reprezentovat paměť jako sekvenci bajtů
a jednotlivé adresy uložené v ukazatelích by následně byly mapovány do tohoto pole bez rozlišování typů.
Kontrola práce s pamětí by pak probíhala na úrovni jednotlivých bajtů.
...
Důkazy by musely být ověřeny manálně pomocí interaktiního důkazového asistenta jako je například Coq~\cite{WPManual}.
...
Axiomatický přístup jako částečné řešení pomocí anotace \texttt{admit} například na vlastnosti,
...
Proměnná \texttt{t} (původně \texttt{Init\_x}) reprezentuje paměťovovou mapu,
...
Platnot predikátu \texttt{finite} jinak nelze potvrdit pro větší seznamy,
...
pokud nový uzel připoojíme na začátek seznamu.
...
\begin{listing}[H]
    ...
    */
    ...
    \end{minted}
    \caption{Dokončení důkazu pomocí pomocí kombinace axiomatického přístupu a oddělené paměti}
...
Serializací přicházíme o lokální pohled na problém a musíme jej nahradit prací s indexy v poli.
...
Převedením dynamické struktury do serializovaného pole zesložiťuje hledání vhodnýho invariant cyklu,
